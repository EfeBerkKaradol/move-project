...
export function RecommendationPanel({
  ...
  return (
    ...
    >
      <div className="rounded-card border-2 border-accent bg-surface p-5">
        ...
        <dl className="mt-4 grid grid-cols-3 gap-3 text-sm">
          <div>
            <dt className="text-xs text-ink-muted">Hacim</dt>
            <dd className="tabular-nums font-medium">{formatVolume(estimate.volumeM3)}</dd>
          </div>
          <div>
            <dt className="text-xs text-ink-muted">Ağırlık</dt>
            <dd className="tabular-nums font-medium">
              {estimate.weightKg.toLocaleString('tr-TR')} kg
            </dd>
          </div>
          <div>
            <dt className="text-xs text-ink-muted">En uzun parça</dt>
            <dd className="tabular-nums font-medium">
          ...
          <ul className="mt-3 space-y-2">
            {suggestedExtras.map((e) => (
              <li key={e.code} className="text-sm">
                ...
                <span className="block text-xs text-ink-muted">{e.reason}</span>
              </li>
            ))}
          </ul>
        </div>
      ...
      <details className="rounded-card border border-line bg-surface p-5">
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          <button
            ...
            className={[
              ...
              overrideCode === null ? 'border-accent bg-accent-soft' : 'border-line',
            ].join(' ')}
          >
            Öneriyi kullan
          </button>
          {vehicleTypes.map((v) => (
            <button
              ...
              className={[
                'rounded-lg border px-3 py-2 text-sm',
                overrideCode === v.code ? 'border-accent bg-accent-soft' : 'border-line',
              ].join(' ')}
            >
              {v.displayName}
            </button>
        ...
        {tooSmall && chosen && (
          <p className="mt-3 rounded-lg border border-line bg-ground px-3 py-2 text-sm">
...
function FillGauge({ percent }: { percent: number }) {
  ...
  return (
    <div className="mt-4">
      ...
      >
        <div
          className={['h-full rounded-full transition-all', tight ? 'bg-primary' : 'bg-accent'].join(
            ' ',
          )}
          style={{ width: `${clamped}%` }}
        />
