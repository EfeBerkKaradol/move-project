...
export function Pill({ tone = 'neutral', children }: { tone?: Tone; children: React.ReactNode }) {
  return (
    <span className={`inline-flex items-center whitespace-nowrap rounded-full px-2.5 py-1 text-xs font-semibold ${TONE[tone]}`}>
      {children}
    </span>
  );
}
...
export function StatCard({
  ...
  const body = (
    <>
      ...
      <p className={`stat mt-2 text-3xl leading-none ${number}`}>{value}</p>
      {context && <p className="mt-2 text-xs text-muted">{context}</p>}
    </>
  );
  const cls = `block rounded-card border bg-surface p-5 shadow-card ${accent}`;
  return href
      ...
      {items.map((f) => {
        ...
        return (
          <Link key={f.value} href={hrefFor(f.value)} aria-current={on ? 'true' : undefined}
            className={`inline-flex min-h-11 items-center gap-2 rounded-field border px-3.5 text-sm font-semibold transition ${
    ...
    <>
      ...
      <ul className="space-y-3 md:hidden">
        {rows.map((row) => {
          ...
          return (
            <li key={rowKey(row)} className="rounded-card border border-line bg-surface p-4 shadow-card">
