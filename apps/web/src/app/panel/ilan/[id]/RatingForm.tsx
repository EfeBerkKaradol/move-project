    ...
    <form action={action} className="space-y-3">
        ...
        <div className="mt-2 flex items-center gap-1" onMouseLeave={() => setHover(0)}>
          {[1, 2, 3, 4, 5].map((n) => (
            <label key={n} className="cursor-pointer" onMouseEnter={() => setHover(n)}>
              ...
              <span aria-hidden className={`grid size-11 place-items-center text-2xl transition ${n <= shown ? 'text-amber' : 'text-line'}`}>★</span>
      ...
      <label className="block text-sm">
        ...
        <input name="comment" maxLength={500} placeholder="Örn. Zamanında geldi, yükü özenle taşıdı"
          className="mt-2 min-h-11 w-full rounded-field border border-line bg-surface-2 px-3 py-2.5 outline-none placeholder:text-muted transition hover:border-muted focus:border-amber focus:ring-2 focus:ring-amber/25" />
      </label>
