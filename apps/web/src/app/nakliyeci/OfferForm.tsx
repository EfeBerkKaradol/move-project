    ...
    <form action={action} className="flex flex-wrap items-end gap-2">
      ...
      <label className="text-sm">
        ...
        <input name="amount" inputMode="decimal" required defaultValue={suggested}
          className="min-h-11 mt-1 w-32 rounded-field border border-line bg-surface-2 px-3 py-2.5 tabular-nums outline-none transition hover:border-muted focus:border-route focus:ring-2 focus:ring-route/25" />
      </label>
      <label className="min-w-0 flex-1 text-sm">
        ...
        <input name="note" maxLength={500} placeholder="Örn. Hamaliye dahil, sabah alırım"
          className="min-h-11 mt-1 w-full rounded-field border border-line bg-surface-2 px-3 py-2.5 outline-none placeholder:text-muted transition hover:border-muted focus:border-route focus:ring-2 focus:ring-route/25" />
      </label>
      <button type="submit" disabled={pending}
        className="min-h-11 rounded-field bg-route px-4 py-2.5 text-sm font-bold text-[var(--route-ink)] disabled:opacity-60 transition hover:bg-[var(--route-hover)] hover:shadow-[0_6px_18px_rgb(244_159_44_/_0.30)] active:translate-y-px">
